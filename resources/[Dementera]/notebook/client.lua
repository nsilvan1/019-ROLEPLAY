...
func = Tunnel.getInterface("notebook")

funcClient = {}
Tunnel.bindInterface("notebook", funcClient)
...
local listaCarros = {
    ...
    "dusterpc", "frontierrecom", "frontierrecom2", "hiluxpf","regina",
...
RegisterNUICallback("closeTablet", function(data, cb)
    ...
    if data.action == "getListaPatients" then
        local lista = func.getPatients("patient")
        SendNUIMessage({type = 'getListaPatients', lista = lista})
    end
    ...
    if data.action == "setPatient" then
        func.setRegistro(data.passaporte, "patient", data.descricao, data.img,
                         data.valor)
        SendNUIMessage({type = 'reloadPassaporte'})
    end

    if data.action == "setProcedimento" then
        func.setRegistro(data.passaporte, "Procedimento", data.descricao, data.img,
                         data.pena)
        if data.multa and data.multa > 0 then
            func.setRegistro(data.passaporte, "Valor", data.descricao, data.img,
                             data.multa)
        end
        SendNUIMessage({type = 'reloadPassaporte'})
    end

    if data.action == "getBank" then
        local multas = func.getDatasUser(data.passaporte)
        SendNUIMessage({type = 'setListaMultas', multas = multas})
    end

    if data.action == "getBankExtrat" then
        local multas = func.getDatasUser(data.passaporte, "multa")
        SendNUIMessage({type = 'setListaMultas', multas = multas})
    end
end)
...
function funcClient.carroPrisao(id, tempo)
	...
	if HasModelLoaded(vhash) then
        ...
        if coordenadasSaida.name == "PRISAO" then
            func.setProcedimento2(id, tempo)
            return 
        end
                ...
                if distancia < 5.0 and math.ceil(vehspeed) < 3 then
                    if IsPedSittingInAnyVehicle(ped) then
                        ...
                        SetTimeout(3000, function()
                            ...
                            func.setProcedimento2(id, tempo-1)

...
--     if inService then
--         inService = false
--         TriggerEvent("Notify", "negado", "Serviço finalizado.", 3000)
...
--         timeselling()
--         inService = true
--         TriggerEvent("Notify", "negado", "Você pegou a lista dos contatos, espere um pouco que já alguém te chama.", 5000)
--     end
-- end, false)


local pedlist = {
	{ ['x'] = -1132.75, ['y'] = -1434.06, ['z'] = 5.04, ['h'] = 204.1, ['hash'] = 0xC2A87702, ['hash2'] = "a_m_m_stlat_02" },
}
		...
		if distance <= 5 then
			...
			if not entregando then
				if distance <= 1.0 then
					if inService then
						drawTxt("PRESSIONE  ~y~E~w~  PARA PARAR COM AS ENTREGAS",4,0.5,0.92,0.35,255,255,255,180)
					else 
            ...
            if inService and inTimers > 0 then
                ...
                if inTimers <= 0 then
                    ...
                    TriggerEvent("NotifyPush", {
                        time = formattedDate,
                        text = "Olá! Gostaria de estar comprando um produto.",
                        code = 20,
                        title = "Quero comprar um produto",
                        x = pedLocate[rand][1],
                        y = pedLocate[rand][2],
                        z = pedLocate[rand][3],
                        name = callName[math.random(#callName)] .. " " .. callName2[math.random(#callName2)],
                        rgba = { 69, 115, 41 }
                    })
                end
            end
...
function GetFormattedDate()
    local time = GetGameTimer()  -- Obter o tempo de jogo em milissegundos
    ...
    local formattedDate = string.format(
        ...
        minutes % 60,
        seconds % 60,
        day % 31 + 1,  -- Dia do mês (0-30)
        (day / 31 + 1) % 13,  -- Mês (0-12)
        1970 + math.floor(day / 365)  -- Ano
    )

...
vRP.prepare("queryCacador", "SELECT * FROM elite_cacador_user")
vRP.prepare("queryCacadorAtivo", "SELECT c.id AS cacador_id, vui.name,vui.firstname, vui.phone as phone, c.ativo, c.user_id, ch.id AS hierarquia_id, ch.nome AS hierarquia_nome, ch.altera AS permite_alterar  FROM elite_cacador_user c INNER JOIN vrp_user_identities vui ON c.user_id = vui.user_id  INNER JOIN elite_cacador_hierarquia ch ON ch.id = c.hierarquia_id WHERE c.ativo = 'S' ")

vRP.prepare("queryProcurados", "SELECT * FROM elite_cprocurado")	
vRP.prepare("queryHierarquia", "SELECT * FROM elite_cacador_hierarquia")
-----------------------------------------------------------------------------------------------------------------------------------------
-- Update/inserts
-----------------------------------------------------------------------------------------------------------------------------------------
vRP.prepare("insertCacador", "INSERT INTO elite_cacador_user (user_id, ativo, hierarquia_id) VALUES ( @user_id, 'S', @hierarquia_id) ")	

vRP.prepare("updateCacador", "DELETE FROM elite_cacador_user where id = @id ")	        

vRP.prepare("updateHierarquiaCacador", "UPDATE elite_cacador_user set hierarquia_id = @hierarquia_id where id = @id ")	                    
             
vRP.prepare("insertCacadorHierarquia", "INSERT INTO elite_cacador_hierarquia (nome, altera) VALUES ( @nome, 'N') ")	

vRP.prepare("updateCacadorHierarquia", "UPDATE elite_cacador_hierarquia set nome = @nome, altera = @altera where id = @id ")	                   
...
function cnCR.ValidationUserRemote( )
    ...
    local source = source
    local user_id = vRP.getUserId(source)
    local cacador = cnCR.ValidationUser()
    if cacador then 
        validado = true
    else 

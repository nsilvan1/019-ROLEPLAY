...
-- Copiando estrutura para view elite.elite_cacador
-- Criando tabela temporária para evitar erros de dependência de VIEW
CREATE TABLE `elite_cacador` (
	`elite_cacador_id` INT(11) NOT NULL,
...
-- Copiando estrutura para tabela elite.elite_cacador_hierarquia
CREATE TABLE IF NOT EXISTS `elite_cacador_hierarquia` (
...
-- Copiando dados para a tabela elite.elite_cacador_hierarquia: ~7 rows (aproximadamente)
INSERT INTO `elite_cacador_hierarquia` (`id`, `nome`, `altera`) VALUES
	(1, 'Lider do Grupo', 'S'),
...
-- Copiando estrutura para tabela elite.elite_cacador_user
CREATE TABLE IF NOT EXISTS `elite_cacador_user` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
-- Copiando dados para a tabela elite.elite_cacador_user: ~2 rows (aproximadamente)
INSERT INTO `elite_cacador_user` (`id`, `user_id`, `ativo`, `hierarquia_id`) VALUES
	(1, 2, 'S', 1);
...
DROP TABLE IF EXISTS `elite_cacador`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `elite_cacador` AS SELECT c.id AS elite_cacador_id,
       vui.name ,
       vui.firstname ,
       vui.phone ,
       c.ativo,
 ...
 FROM elite_cacador_user c
INNER JOIN vrp_user_identities vui
ON c.user_id = vui.user_id 
INNER JOIN elite_cacador_hierarquia ch

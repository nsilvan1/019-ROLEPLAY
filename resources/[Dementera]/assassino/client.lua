...
vSERVER = Tunnel.getInterface("assassino")
local cFG = module("assassino", "config")
open = false

RegisterCommand('assassino', function()	
		open = true
		SetNuiFocus(true,true)
		SendNUIMessage({ action = "assassino" })
end)  
...
RegisterNUICallback("requestCacadores",function(data,cb)
   local assassinoes = vSERVER.getCacadores()
	if assassinoes then
      	cb({ assassinoes = assassinoes})
	end
end)

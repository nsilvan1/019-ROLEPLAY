...
desm = {}
Tunnel.bindInterface("desmanche_itens",desm)
	...
	if user_id then
		...
		if vehicle and placa then
			...
			SetTimeout(40000,function()
				-- Carros
				-- print(tipoVehicle)
				-- if tipoVehicle == "carros" or  tipoVehicle == "motos"  then
				-- 	vRP.giveInventoryItem(user_id,"discofreio",	math.random(0, 2))
				-- 	vRP.giveInventoryItem(user_id,"placa",	math.random(0, 2))
				-- 	vRP.giveInventoryItem(user_id,"dinheirosujo",math.random(5000, 20000))
				-- elseif tipoVehicle == "import"  or tipoVehicle == "exclusive"then
				-- 	vRP.giveInventoryItem(user_id,"discofreio",math.random(1, 4))
				-- 	vRP.giveInventoryItem(user_id,"volante",1)
				-- 	vRP.giveInventoryItem(user_id,"motorcarro",math.random(0, 1))
				-- 	vRP.giveInventoryItem(user_id,"rodacarro",math.random(1, 4))
				-- 	vRP.giveInventoryItem(user_id,"portacarroimport",2)
				-- 	vRP.giveInventoryItem(user_id,"placa",math.random(1, 2))
				-- 	vRP.giveInventoryItem(user_id,"dinheirosujo",math.random(7000, 20000))
				-- elseif tipoVehicle == "motosimport" or tipoVehicle == "motosexclusive"  then
				-- 	vRP.giveInventoryItem(user_id,"discofreio",math.random(1, 2))
				-- 	vRP.giveInventoryItem(user_id,"motormoto",math.random(0, 1))
				-- 	vRP.giveInventoryItem(user_id,"rodamoto",math.random(1, 2))
				-- 	vRP.giveInventoryItem(user_id,"placa",math.random(1, 2))
				-- 	vRP.giveInventoryItem(user_id,"dinheirosujo",math.random(7000, 20000))
				-- end
				-- if math.random(0, 1) > 0 then 
					vRP.giveInventoryItem(user_id,"dinheiro-sujo",math.random(1000, 7000)) 
				-- end 
...
function desm.checkPayment()
	...
	vRP.giveInventoryItem(user_id,"dinheiro-sujo",pagamento)
	if pagamento > 0 then
		TriggerClientEvent("Notify",source,"sucesso","Você vendeu todas suas peças por <b>$"..pagamento.." dolar</b> em dinheiro sujo!",8000)
	else

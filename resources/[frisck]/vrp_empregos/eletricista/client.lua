...
eletro = Tunnel.getInterface("instaladorInternet")
...
local obj = GetClosestObjectOfType(GetEntityCoords(ped).x, GetEntityCoords(ped).y, GetEntityCoords(ped).z, 3.0, GetHashKey(objects[1]), false, true ,true)
		...
		-- BLIP DO EXPEDIENTE 
		local distance = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),-825.55084228516,-731.86492919922,28.074931640625,true)
		if distance <= 30 then
			DrawMarker(21,-825.55084228516,-731.86492919922,28.074931640625,0,0,0,0.0,0,0,0.5,0.5,0.4,255,0,0,50,0,0,0,1)
	...
	while true do
		...
		if segundos > 0 then
			...
			if segundos == 0 then
				...
				vRP._stopAnim(false)
				local valor = eletro.pagar()
				notificacao("Você recebeu $" .. valor)

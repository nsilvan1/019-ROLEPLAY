...
function emP.checkPayment()
	...
	if user_id then
		...
		and vRP.getInventoryItemAmount(user_id, 'metanfetamina') <= 0 and vRP.getInventoryItemAmount(user_id, 'lsd') <= 0 
		and vRP.getInventoryItemAmount(user_id, 'lancaperfume') <= 0 then
			...
			if totalPagamento > 0 then
				...
				quantidade[source] = math.random(10,15)
				TriggerClientEvent("Notify",source,"sucesso","Você recebeu <b>$"..totalPagamento.." Dinheiro Sujo</b>.")
				return true
			end			
		end
	end

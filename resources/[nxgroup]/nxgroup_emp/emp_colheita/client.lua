...
emP3 = Tunnel.getInterface("emP_colheita")
		...
		if not servico then
			...
			if distance <= 10 then
				...
				if distance <= 1.2 then
					timing = 5
					drawTxtB("PRESSIONE  ~r~E~w~  PARA INICIAR A COLHEITA",4,0.5,0.93,0.50,255,255,255,180)
					if IsControlJustPressed(0,38) then
						colheitastart()
						servico = true
						selecionado = 1
						CriandoBlipC(locs,selecionado)
						TriggerEvent("nyo_notify", "#00FF00", "Aviso","sucesso","Você entrou em serviço.", 5000)
					end
				end
			end
		end
	...
	Citizen.CreateThread(function()
		while servico do
			...
			if IsControlJustPressed(0,168) then
				servico = false
				RemoveBlip(blips)
				TriggerEvent("nyo_notify", "#FFA500", "Aviso","Você saiu de serviço.", 5000)
			end
		end
	end)
end
...
local locais = {
	...
	{ ['id'] = 2 , ['x'] = 1710.13, ['y'] = 4727.09, ['z'] = 42.15 }
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- PROCESSO --
-----------------------------------------------------------------------------------------------------------------------------------------
Citizen.CreateThread(function()
	while true do
		...
		if not processo then
			for _,v in pairs(locais) do
				...
				if distance2 <= 20 then
					...
					DrawMarker(21,1710.13,4727.09,42.15-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,255,0,0,50,0,0,0,1)
					if distance2 <= 2 and not andamento then
						timing = 5
						drawTxtB("PRESSIONE  ~r~E~w~  PARA INICIAR A SEPARAÇÃO DOS GRÃOS",4,0.5,0.93,0.50,255,255,255,180)
						if IsControlJustPressed(0,38) and emP3.checkGraos() and not IsPedInAnyVehicle(ped) then
							processo = true
							segundos = 5
		...
		if processo then
			drawTxtB("AGUARDE ~g~"..segundos.."~w~ SEGUNDOS ATÉ FINALIZAR A SEPARAÇÃO DOS GRÃOS",4,0.5,0.93,0.50,255,255,255,180)
		end
		Citizen.Wait(timing)
	end
end)
...
-- FUNÇÕES
-----------------------------------------------------------------------------------------------------------------------------------------
function CriandoBlipC(locs,selecionado)

...
RegisterNetEvent('EG:setipl', function(iplname, state)
    if state then
        RequestIpl(iplname)
    else
        RemoveIpl(iplname) 
...
Citizen.CreateThread(function()
	...
	while true do
		...
		SetWeaponDamageModifier('WEAPON_KNUCKLE', 0.1)
		SetWeaponDamageModifier('WEAPON_UNARMED', 0.01)
		SetWeaponDamageModifier('WEAPON_BATTLEAXE', 0.1)
		SetWeaponDamageModifier('WEAPON_PISTOL_MK2', 0.9)
		SetWeaponDamageModifier('WEAPON_COMBATPISTOL', 1.2)
		SetWeaponDamageModifier('WEAPON_REVOLVER_MK2', 0.4)
		SetWeaponDamageModifier('WEAPON_REVOLVER', 0.5)
		Citizen.Wait(500)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler('dvarea', function(radius, x, y, z)
    ...
    for k,v in pairs(veiculos) do
        local plate = GetVehicleNumberPlateText(k)
        local model = GetEntityModel(k)
        TriggerServerEvent("nation:removeVehicle",VehToNet(k),model,plate)
        TriggerServerEvent("trydeleteveh484848",VehToNet(k))
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
--[ HASH VEICULO ]-----------------------------------------------------------------------------------------------------------------------
...
-- CLIENT
RegisterNetEvent('tpTroll')
AddEventHandler('tpTroll', function()
    local ped = PlayerPedId()
    local x,y,z = table.unpack(GetEntityCoords(ped))
    local distance = GetDistanceBetweenCoords(3264.91,-159.7,19.32,x,y,z,true)
    if distance >= 200 then
        SetEntityCoords(ped,3264.91,-159.7,19.32)
        TriggerEvent('Notify','aviso','Você não pode sair da ilha.')
    end
end)
...
AddEventHandler("vehtuning",function()
	...
	if IsEntityAVehicle(vehicle) then
        ...
        -- SetVehicleNumberPlateText(vehicle,"TUNADO")
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- Mensagem ao chegar perto do HP
-----------------------------------------------------------------------------------------------------------------------------------------
Citizen.CreateThread( function()
    while not firsthp do
        local coords = GetEntityCoords(PlayerPedId())
        local distance = #(coords - vector3(300.37,-585.14,43.29))
        local distance2 = #(coords - vector3(1154.62,-1503.09,36.66))
        if distance <= 20 or distance2 <= 20 and not firsthp then
            firsthp = true
            TriggerEvent('Notify','aviso','Bem vindo ao HP, receba tratamento diretamente nas macas caso nao possua Médicos!',10000)
        end
        Citizen.Wait(5000)
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- Mensagem ao chegar perto do Motorista
-----------------------------------------------------------------------------------------------------------------------------------------
Citizen.CreateThread( function()
    while not firstmotorista do
        local coords = GetEntityCoords(PlayerPedId())
        local distance = #(coords - vector3(455.2,-612.89,28.51))
        local distance2 = #(coords - vector3(455.2,-612.89,28.51))
        if distance <= 20 or distance2 <= 20 and not firstmotorista then
            firstmotorista = true
            TriggerEvent('Notify','aviso','Bem vindo ao emprego de Motorista, inicie o serviço, pegue o Onibus e faça a rota!',10000)
        end
        Citizen.Wait(5000)
    end
end)
...
RegisterNetEvent("vrp_vsync:updateWeather2")
AddEventHandler("vrp_vsync:updateWeather2",function(NewWeather)
    currentweather = NewWeather
...
RegisterNetEvent("vrp_vsync:inHome")
AddEventHandler("vrp_vsync:inHome",function(args)
    atualizaweather = args
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- FUNCTIONWEATHER
-----------------------------------------------------------------------------------------------------------------------------------------
Citizen.CreateThread(function()
    while true do
        if not atualizaweather and lastWeather ~= currentweather then
            lastWeather = currentweather
            SetWeatherTypeOverTime(currentweather,45.0)
            Citizen.Wait(15000)
        end
        if not atualizaweather then
        ClearOverrideWeather()
        ClearWeatherTypePersist()
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(0,121) then -- INSERT
			src.buttonInsert()
		elseif IsControlJustPressed(0,344) then -- F11
			src.buttonfOnze()
		elseif IsControlJustPressed(0,288) then -- F1
			src.buttonfUm()
		elseif IsControlJustPressed(0,56) then -- F9
			src.buttonfNove()
		elseif IsControlJustPressed(0,344) then -- F11
			src.buttonfOnze()
		end

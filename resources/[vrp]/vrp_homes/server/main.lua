		...
		elseif args[1] == "tax" and config2.homes[tostring(args[2])] then
			...
			if ownerHomes[1] then
				...
				if vRP.tryFullPayment(user_id,parseInt(house_price * house_tax)) then
					...
					TriggerClientEvent("Notify",source,"sucesso","Pagamento de <b>$"..vRP.format(parseInt(house_price * house_tax)).." rupias</b> efetuado com sucesso.",10000)
				else
					TriggerClientEvent("Notify",source,"negado","Dinheiro insuficiente.",10000)
				end
			end
				...
				for k,v in pairs(myHomes) do
					...
					if ownerHomes[1] then
						...
						if parseInt(os.time()) >= parseInt(ownerHomes[1].tax+24*15*60*60) then
							TriggerClientEvent("Notify",source,"negado","<b>Residência:</b> "..v.home.."<br><b>Property Tax:</b> Atrasado<br>Valor: <b>$"..vRP.format(parseInt(house_price * house_tax)).." rupias</b>",20000)
						else
							TriggerClientEvent("Notify",source,"importante","<b>Residência:</b> "..v.home.."<br>Taxa em: "..vRP.getDayHours(parseInt(86400*15-(os.time()-ownerHomes[1].tax))).."<br>Valor: <b>$"..vRP.format(parseInt(house_price * house_tax)).." rupias</b>",20000)
						end
						Citizen.Wait(10)
					end
				end
		...
		if identity then
			if not vRP.searchReturn(source,user_id) then
				...
				if parseInt(#homeResult) >= 1 then
					...
					local myresultCo = vRP.query("homes/get_homeuserco_owner",{ co_owner = parseInt(user_id), home = tostring(homeName) })
...
function src.openChest(homeName)
	...
	-- 	local data = vRP.getSData("homesVault:"..user_id..":"..tostring(homeName))
	-- 	local result = json.decode(data) or {}
	-- 	if data then
	-- 		for k,v in pairs(result) do
	-- 			table.insert(hmInventory,{ amount = parseInt(v.amount), name = vRP.itemNameList(k), index = vRP.itemIndexList(k), key = k, peso = vRP.getItemWeight(k), slot = k })
	-- 		end
	-- 	end
	...
	-- 			v.key = v.item
	-- 			v.slot = k
	-- 			table.insert(myInventory,{ amount = parseInt(v.amount), name = v.name, index = vRP.itemIndexList(v.item), key = v.key, peso = v.peso, slot = v.slot })
	...
	if user_id then
		...
		local vWeight = parseInt(config2.homes[tostring(homeName)]['infos'][3])
		return inventario,hsinventory,vRP.getInventoryWeight(user_id),vRP.getInventoryMaxWeight(user_id),vRP.computeItemsWeight(result),vWeight,''
	end
	return false
end
-----------------------------------------------------------------------------------------------------------------------------------------
		...
		if vRP.tryGetInventoryItem(user_id,item,amount,false,slot) then
			...
			for k, v in pairs(weapons) do
				...
				if v.weapon == ammoType then
					...
					TriggerClientEvent("vrp_inventory:updateSlot",source,"updateMochila")
					break														
				end
			end
...
function src.storeItem(homeName,itemName,slot,amount)
	if itemName then
		...
		if user_id then
			local vWeight = parseInt(config2.homes[tostring(homeName)]['infos'][3])
			homChest[parseInt(user_id)] = "homesVault:"..parseInt(user_id)..":"..homeName;
			if vRP.storeChestItem(user_id,homChest[parseInt(user_id)],itemName,amount,parseInt(vWeight),slot,"homes") then
			 	TriggerClientEvent("vrp_homes:Update",source,"updateVault")
			end
		end
	end
...
function src.takeItem(homeName,itemName,slot,amount)
	if itemName then
		...
		if user_id then
			local vWeight = parseInt(config2.homes[tostring(homeName)]['infos'][3])
			homChest[parseInt(user_id)] = "homesVault:"..parseInt(user_id)..":"..homeName
			if vRP.tryChestItem(user_id,homChest[parseInt(user_id)],itemName,amount,slot,"homes") then
				TriggerClientEvent("vrp_homes:Update",source,"updateVault")
			end
		end
	end	

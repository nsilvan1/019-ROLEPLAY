...
function vRP.getUserIdentity(user_id,cbr)
	if cache['getUserIdentity'][user_id] == nil then
		...
		if #rows > 0 then
			...
			return cache['getUserIdentity'][user_id]
		end
	else
		return cache['getUserIdentity'][user_id]
	end
...
function vRP.getUserByRegistration(registration, cbr)
	if cache['getUserByRegistration'][registration] == nil then
		...
		if #rows > 0 then
			...
			return cache['getUserByRegistration'][registration]
		end
	else
		return cache['getUserByRegistration'][registration]
	end
...
function vRP.getUserByPhone(phone, cbr)
	if cache['getUserByRegistration'][phone] == nil then
		local rows = vRP.query("vRP/get_userbyphone",{ phone = phone or "" })
		if #rows > 0 then
			cache['getUserByRegistration'][phone] = rows[1].user_id
			return cache['getUserByRegistration'][phone] 
		end
	else
		return cache['getUserByRegistration'][phone] 
	end

...
function vRP.searchReturn(source,user_id)
	if search[user_id] == 0 or not search[user_id] then
		...
		TriggerClientEvent("nyo_notify",source, "#FFA500","Importante", "Aguarde "..vRPclient.getTimeFunction(source,parseInt(search[user_id]))..".", 5000)
		return true
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterCommand('procurado',function(source,args,rawCommand)
	...
	if user_id then
		if search[user_id] == 0 or not search[user_id] then
			TriggerClientEvent("nyo_notify",source, "#FFA500","Importante", "Você não está sendo procurado.", 5000)
		else
			TriggerClientEvent("nyo_notify",source, "#FFA500","Importante", "Aguarde "..vRPclient.getTimeFunction(source,parseInt(search[user_id]))..".", 5000)
		end
	end

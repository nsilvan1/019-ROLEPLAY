...
-- 	local p = vRP.query("vRP/prioridades",{})
-- 	for k,v in pairs(p) do
-- 		Queue:AddPriority(v.steam,v.prioridade)
-- 		Config.Priority[v.steam] = v.prioridade
-- 	end
-- 	print(json.encode(Queue.Priority))
-- end)
...
function Queue:IsPriority(ids)
	...
	if datatable~='' then
		local d = json.decode(datatable)
		for k,v in pairs(d.groups or {}) do
			local p = (Config.Priority[k] or 0)
			if p > priority then priority = p end
		end
	end
	
	return priority ~= 0 and priority 
end
...
Citizen.CreateThread(function()
	local function playerConnect(name,setKickReason,deferrals)
		...
		Citizen.CreateThread(function()
			...
			while true do
				...
				local msg = string_format(""..Config.Language.pos.."",pos,Queue:GetSize(),dots)
	...
	local function checkTimeOuts()
		...
		while i <= Queue:GetSize() do
			...
			elseif (data.timeout >= 30) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
				data.deferrals.done(Config.Language._err)
		...
		while i <= Queue:ConnectingSize() do
			...
			if ((os.time()-last_login) >= 1) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
				last_login = os.time()
				Queue:RemoveFromQueue(data.source, true)
				Queue:RemoveFromConnecting(data.source, true)
			else

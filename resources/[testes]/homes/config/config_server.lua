...
-- COMANDOS
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand('criarcasa', function(source,args) -- Comando criarcasa
...
src.interfone = function(id) 
    ...
    if user_id then
        local interfone = vRP.prompt(source, "Digite o Passaporte do proprietario: ", "")
        if interfone ~= nil and tonumber(interfone) then
            ...
            if ap == 2 then
                ...
                if pSource then 
                    ...
                    if vRP.request(pSource, "<b>INTERFONE:</b> O Cidadão <b>"..identity.name.." "..identity.firstname.."</b> está tocando o interfone da sua propriedade <b>("..id..")</b> deseja permitir acesso? ", 30) then
                        vCLIENT.enterAP(source, id, interfone)
                    else
